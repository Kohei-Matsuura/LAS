...
def load_dat_htk(filename, spec):
    """
...
def make_pps_batches(line_list, bs, line_num, iteration, sp):
    #st = time.time()
    bx, bt = list_to_batches(line_list, bs, line_num, iteration, sp)
...
def make_pps_batches_with_names(line_list, bs, line_num, iteration, sp):
    #st = time.time()
    bx, bt, n = list_to_batches_with_names(line_list, bs, line_num, iteration, sp)
    #print('check1: ' + str(time.time() - st))
...
def list_to_batches(line_list, bs, line_num, iteration, sp):
    """
    Input:
    ...
    if iteration < int(line_num / bs):
        for i in range(bs):
            line = bs * iteration + i
            batch_x.append(load_dat(line_list[line][0], sp))
...
def batch_to_pps(x_batch, t_batch):
    ...
    x_tensor = torch.tensor(padded_x_batch, dtype=torch.float32, requires_grad=True).to(DEVICE)
    #print(x_tensor.size())
    #print(x_lengths)
    x_pps = nn.utils.rnn.pack_padded_sequence(x_tensor, x_lengths, batch_first=True)
    x_lengths = torch.IntTensor(x_lengths).to(DEVICE)
    #
    # ---------- Processing t (= label) ----------
    #
    t_lengths = []
    ...
    t_tensor = torch.tensor(padded_t_batch).to(DEVICE)
    t_lengths = torch.IntTensor(t_lengths).to(DEVICE)
    ...
    for i in range(len(targets)):
        for j in range(len(targets[0])):
            ...
            if j < tl[i]:
                ...
                # padding
                pass
    onehot_targets = torch.FloatTensor(onehot_targets).to(DEVICE)
...
def batch_softmax(b):
    t = torch.zeros(b.shape).to(DEVICE)
...
def batch_to_lengths(batch):
    ...
    for e in batch:
        lens.append(len(e))
    return torch.IntTensor(lens).to(DEVICE)
